...
binding regions'. We used the normalized difference score to find
binding regions and we also look at other techniques for ChIP-seq
data analysis.
\end{abstract}
...
Then the tpta; number of tags to be called a peak is calculated as
...
(Figure). The normalization model is extrapolated to all peaks, and
the normalized read count is used to find conserved binding sites.
Additionally a bayesian Poisson model is used to consider data from
...
Finally, another normalization method was the normalized difference
score from Zheng et al. We will review this model in the discussion
of this paper.
...
'binding signal' (see methods section 2.1) and they analyzed the variation
of NormDiff scores to identify 'variable binding regions'. They identified
cis- and trans- factors that contribute to binding variation using
a QTL mapping technique called single marker regression to correlate
'variable binding regions' with genetic markers. The cis-factors were
identified as genetic markers that are close to the variable binding
regions whereas the trans-factors were identified as genetic markers
that are far away from the binding sites that were still correlatedd
with binding variation

They identified trans-factors using the QTL analysis but they also
aimed to identify the trans-acting genes responsible for the binding
differences. They identified that markers to be closely associated
with target genes in two cases (FLO8 and AMN1), and they found trans-clusters
for different sets of individuals that validated the QTL results.
They tested the genes they identified in knockout methods to test
for binding variations.\\
\\
