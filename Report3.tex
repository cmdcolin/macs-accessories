...
helped us understand the details of ChIP-seq processing. 

MACS is a command line program that has a number of configurability
options. To understand these options it is important to understand
the processing that MACS does. First MACS loads ChIP-seq tags and
builds an internal representation in a standard Python hash data structure
called a dictionary. The dictionary associates the chromosome as the
key and the position and orientation as the value. It does not keep
the details about mismatches in alignment or any sequence data that,
for example, is stored with ELAND file inputs. 

After reading in the ChIP-seq data, MACS calculates what the so called
'shift-model' for paired-end reads. The shift mode calculates the
offset from sequencing the 5' ends of the fragments on +/- strands,
and shifts them toward the 3' end in order to compensate. To calculate
the shiftsize 'd' MACS uses a ``naive'' peak finder to get peaks
that are at least MFOLD greater than the background. Then the median
of the distance between the top of the peaks is called 'd' and reads
are shifted by d/2 in the 3' direction.

{*}It is possible that additional analysis could reveal additional
information about mismatch alignments that correlate with data features
