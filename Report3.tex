...
The genetic code in our DNA is augmented by a variety of proteins
that regulate . Transcription factors are proteins which recognize
and bind to short DNA sequences, and they are important for regulating
DNA transcription. Chromatin immunoprecipitation (ChIP) can be used
to analyze transcription factor binding sites and identify genetic
variations by comparing binding sites from different individuals.
...
using ``peak calling''. Zheng et al. characterized genetic differences
in yeast that caused transcription factor binding variations by using
a normalized difference score to find 'variable binding regions'.


\section{Background}

Here we will look at some background papers for analyzing transcription
factor binding sites and methods for finding conserved binding sites
and differential sites. ChIP-seq on the Illumina platform is done
using bridge PCR. For this method, ChIP-seq fragments are attached
to adapters and bridged on a bead array. The resulting 5' ends of
the fragments are sequenced. Since only the ends of the fragments
are sequenced and base-calls are done using image analysis. Reads
are sequenced to the genome using alignment algorithms that sequence
a large number of reads. Then we perform peak calling algorithms to
process reads and find binding sites
