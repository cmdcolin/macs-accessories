...
interacting with other TF including RNA polymerase. Chromatin immunoprecipitation
(ChIP) can be used to find transcription factor binding sites experimentally.
ChIP uses antibodies that crosslink transcription factors or other
proteins with DNA to immunoprecipitate protein-DNA complexes. ChIP
with high throughput sequencing (ChIP-seq) or oligonucleotide tiling
arrays (ChIP-chip) can identify transcription factor binding sites
using ``peak calling''. We can also compare the patterns of transcription
factor binding across individuals and show genetic differences that
underlie the causes of the variations. Zheng et al. characterized
genetic differences in yeast that caused transcription factor binding
variations by using a normalized difference score to find 'variable
binding regions'.
...
The platform we use from Illumina uses some proprietary technology
for their Genome Analyzer and Solexa Sequencing. In their process
the libraryHere the ChIP seq fragments are attached to adapters and
go through bridge PCR to construct a fragment library. The 5' ends
of the fragments are sequenced and base-calls are done using image
analysis. Since many reactions are done in paralell we call it high-throughput
sequencing. Since only the ends of the reads are sequenced, the position
of the binding sites is estimated by extending fragments in the 3'
direction. Reads are sequenced to the genome using alignment algorithms
that sequence a large number of reads. Then peak calling algorithms
to find ChIP-seq enriched genomic regions over the background distribution.


\subsection{Model based analysis of ChIP-seq (Zhang et al. 2007)}

ChIP-seq machines such as Illumina data gives us many short read sequences
that are fragments from the end of ChIP-seq reads. program identifies
transcription factor binding sites in ChIP-seq data by using peak
finding. It takes a large number of ChIP-seq reads and analyzes over-enriched
areas according to the distribution of over the background. We also
use computational methods in order to compensate and for the offset
from the ends of fragments to find the true binding site positions.
(Continued)
